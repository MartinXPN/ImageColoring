...
from keras.layers import Conv2D, MaxPooling2D, Concatenate, UpSampling2D, LeakyReLU, Activation


class Colorizer(Model):
    def __init__(self, input_shape=(None, None, 1),
                 feature_extractor_model_path=None, train_feature_extractor=False, feature_extractor_activation='relu',
        ...
        for filters, num_layers in zip([64, 128, 256, 512, 512], [2, 2, 3, 3, 3]):
            ...
            for i in range(num_layers):
                ...
                x = Activation(feature_extractor_activation)(x) if type(feature_extractor_activation) is str else \
                    feature_extractor_activation()(x)
            x = MaxPooling2D((2, 2), strides=(2, 2))(x) if feature_extractor_model_path else \
                Conv2D(filters=filters // 2, kernel_size=3, strides=2, padding='same')(x)
        ...
        for filters, concat_layer, num_layers in zip([256, 256, 128, 64, 32], concat_layers, [3, 3, 3, 2, 2]):
            for i in range(num_layers):
